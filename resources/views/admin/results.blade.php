...
<div class="container-fluid">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        @if($isBatchTwoView)
                            ...
                            <td>
                                @if($student->batch_two_rank)
                                    <span class="badge-soft badge-soft-industry">#{{ $student->batch_two_rank }}<small>/{{ $student->batch_two_rank_total }}</small></span>
                                @else
                                    <span class="text-muted small">-</span>
                                @endif
                            </td>
                        ...
                        <td>
                            ...
                            @elseif($student->batch_two_ct_result)
                                <span class="badge-soft badge-soft-status-done" style="background:#f0f9ff;color:#0369a1;border-color:#bae6fd;"><i class="fas fa-brain"></i> Selesai (CT)</span>
                            @else
                        ...
                        <td>
                            ...
                            @elseif($student->batch_two_ct_result)
                                <div class="d-flex flex-column gap-1">
                                    <span class="fw-bold text-dark" style="font-size: 0.85rem;">{{ $student->batch_two_ct_result->rekomendasi }}</span>
                                    <span class="badge-soft badge-soft-status-done" style="padding: 0.1rem 0.4rem; font-size: 0.6rem; background:#f0fdf4; color:#166534;">Batch 2 CT</span>
                                </div>
                            @else
                        ...
                        <td class="text-center">
                            ...
                            @elseif($student->batch_two_ct_result)
                                @php
                                    $ct = $student->batch_two_ct_result;
                                    $maxCt = max($ct->persen_web, $ct->persen_marketing, $ct->persen_admin);
                                @endphp
                                <span class="score-text text-info">{{ number_format($maxCt, 1) }}%</span>
                            @else
                        ...
                        <td>
                            <div class="d-flex justify-content-center gap-2">
                                ...
                                @elseif($student->batch_two_ct_result)
                                    <button type="button" class="btn btn-sm btn-light border text-info" data-bs-toggle="modal" data-bs-target="#ctModal{{ $student->id }}" title="Lihat Hasil CT">
    ...
    @foreach($students as $student)
        ...
        @if(!$submission && $student->batch_two_ct_result)
            @php $ct = $student->batch_two_ct_result; @endphp
            <div class="modal fade" id="ctModal{{ $student->id }}" tabindex="-1" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content border-0 shadow">
                        <div class="modal-header bg-info bg-opacity-10">
                            <h5 class="modal-title text-info fw-bold">
                                <i class="fas fa-brain"></i> Hasil Batch 2 CT - {{ $student->full_name }}
                            </h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            <div class="result-meta-card mb-4">
                                <div class="text-muted small uppercase fw-bold mb-1">Rekomendasi Utama (CT)</div>
                                <div class="fw-bold text-info fs-5">{{ $ct->rekomendasi }}</div>
                                <div class="text-muted small mt-1">Disubmit pada: {{ optional($ct->submitted_at)->format('d M Y, H:i') ?? '-' }}</div>
                            </div>

                            <h6 class="fw-bold mb-3">Detail Skor Bidang:</h6>
                            <div class="mb-3">
                                <div class="d-flex justify-content-between mb-1">
                                    <span class="small fw-bold">Pemrograman</span>
                                    <span class="small fw-bold text-primary">{{ number_format($ct->persen_web, 1) }}%</span>
                                </div>
                                <div class="progress" style="height: 6px;">
                                    <div class="progress-bar" style="width: {{ $ct->persen_web }}%"></div>
                                </div>
                            </div>
                            <div class="mb-3">
                                <div class="d-flex justify-content-between mb-1">
                                    <span class="small fw-bold">Digital Marketing</span>
                                    <span class="small fw-bold text-success">{{ number_format($ct->persen_marketing, 1) }}%</span>
                                </div>
                                <div class="progress" style="height: 6px;">
                                    <div class="progress-bar bg-success" style="width: {{ $ct->persen_marketing }}%"></div>
                                </div>
                            </div>
                            <div class="mb-3">
                                <div class="d-flex justify-content-between mb-1">
                                    <span class="small fw-bold">Administrasi</span>
                                    <span class="small fw-bold text-danger">{{ number_format($ct->persen_admin, 1) }}%</span>
                                </div>
                                <div class="progress" style="height: 6px;">
                                    <div class="progress-bar bg-danger" style="width: {{ $ct->persen_admin }}%"></div>
                                </div>
                            </div>
                        </div>
